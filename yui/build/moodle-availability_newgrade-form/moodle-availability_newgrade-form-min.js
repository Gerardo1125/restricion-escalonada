YUI.add('moodle-availability_newgrade-form', function (Y, NAME) {
    ...
    M.availability_newgrade.form.getNode = function (json) {
        ...
        var gradeItem = this.grades.find(g => g.id === json.id);
        var minGrade = (gradeItem.grademax - gradeItem.grademin) * (json.min / 100) + parseInt(gradeItem.grademin);
        var maxGrade = (gradeItem.grademax - gradeItem.grademin) * (json.max / 100) + parseInt(gradeItem.grademin);
        ...
        if (json.min !== undefined) {
            ...
            node.one('input[name=minval]').set('value', minGrade);
        }
        if (json.max !== undefined) {
            node.one('input[name=max]').set('checked', true);
            node.one('input[name=maxval]').set('value', maxGrade);
        }
    ...
    M.availability_newgrade.form.fillValue = function (value, node) {
        ...
        if (node.one('input[name=min]').get('checked')) {
            value.min = this.getValue('minval', node, value.id);
        }
        if (node.one('input[name=max]').get('checked')) {
            value.max = this.getValue('maxval', node, value.id);
        }
    };
     ...
     */
    M.availability_newgrade.form.getValue = function (field, node, i) {
        // Get field value.
        var value = node.one('input[name=' + field + ']').get('value');

        var gradeItem = this.grades.find(g => g.id === i);
        ...
        //     return result;
        // }
        if (result < gradeItem.grademin || result > gradeItem.grademax) {
            return value;
        } else {
            return (result - gradeItem.grademin) / (gradeItem.grademax - gradeItem.grademin) * 100;
        }
    };

 ...
 * Front-end class.
 *
 * @package availability_newgrade
 * @copyright 2014 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace availability_newgrade;
...
class frontend extends \core_availability\frontend
{
    ...
    protected function get_javascript_init_params(
        ...
        foreach ($items as $id => $item) {
            ...
            if (
                $cm && $cm->instance == $item->iteminstance
                && $cm->modname == $item->itemmodule
                && $item->itemtype == 'mod'
            ) {
                continue;
            }
            $gradeoptions[$id] = $item->get_name(true) . '_' . $item->grademin . '_' . $item->grademax;
        ...
        foreach ($gradeoptions as $id => $name) {
            ...
            $jsarray[] = (object)array('id' => $id, 'name' => $attrs[0], 'grademin' => $attrs[1], 'grademax' => $attrs[2]);
        }
